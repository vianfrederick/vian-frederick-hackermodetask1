...
function stopWatch() {
  ...
  localStorage.setItem("SECONDSMEDIUM", seconds);
  localStorage.setItem("MILLISECONDSMEDIUM", milliSeconds);
  nettime = (seconds*1000) + milliSeconds;
  localStorage.setItem("NETTIMEMEDIUM", nettime);
}
...
document.querySelector(".results2").addEventListener("click",localStorage.setItem("count6MEDIUM", count61));
...
function displayResults(){
  ...
  setTimeout(function(){
  window.location.replace("MEDIUMResults.html");
}, 100);
}
